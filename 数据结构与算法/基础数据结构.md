...
堆又叫做优先队列，是一种特殊的完全二叉树，规则与队列类似，通常可用数组实现
...
强烈建议参考：[Java实现并查集](https://blog.csdn.net/qq_42011541/article/details/83378709)
...
weighted优化：在合并操作时，节点多的作为根，节点少的作为子树  
路径压缩(path compression)优化：把所有元素直系代表元素的节点作为其直接的上一层父节点（如上图的0--->1--->3压缩为0--->3）
...
**线性建堆法**将建堆操作的时间复杂度从O($n\log_2n$)降为O(n)，但整体堆排序的复杂度仍是O($n\log_2n$)
