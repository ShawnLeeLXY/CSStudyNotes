...
Redis会把设置了过期时间的key放入一个独立的字典里，在key过期时不会立刻删除

过期策略：

- **惰性删除**：key被客户端访问时，Redis会检查该key是否过期，若过期则删除
- **定期扫描**：Redis默认执行10次过期扫描（可配置），步骤如下：
  1. 从过期字典中随机选择20个key
  2. 删除这20个key中已过期的key
...
当Redis占用内存超出 `maxmemory` 时，Redis将采用设置的 `maxmemory-policy` 来淘汰一些数据以释放一定的空间

可设置的淘汰策略：

- noeviction：对可能导致增大内存的命令返回错误
- volatile-ttl：在设置了过期时间的key中，选择甚于寿命最短的key淘汰
- volatile-lru：在设置了过期时间的key中，选择最近最久未使用的key淘汰
- volatile-random：在设置了过期时间的key中，随机选择一些key淘汰
- allkeys-lru：在所有的key中，选择最近最久未使用的key淘汰
...
- 给每个key维护一个时间戳，淘汰时随机采样5个key，从中淘汰掉最旧的key；如果仍然超出内存限制，则继续随机采样淘汰
- 比LRU算法节约内存，效果却非常近似
...
**缓存穿透**就是**大量请求的key不在缓存中**，导致请求直接到了数据库上，可能导致黑客攻击

解决方法：

1. **前端限制**：前端设置请求格式、ID范围等限制
2. **缓存空对象**：缓存无效（缓存和数据库均无数据）的key，并设置较短的过期时间（适用于请求的key变化不频繁的情况）
3. **布隆过滤器**：使用布隆过滤器，先判定请求值是否存在于布隆过滤器中，存在则再去查缓存，否则是无效请求
...
**缓存击穿**指的是热点数据缓存失效瞬间，大量请求直达存储层，导致服务崩溃
...
1. **加互斥锁**：对线程访问的数据加互斥锁
2. **永不过期**：对热点数据的key设置永不过期，对value设置逻辑过期时间，当发现该值逻辑过期时，使用单独的线程重建缓存
...
**缓存雪崩**指的是缓存在**同一时间**大面积的失效（或缓存层不能提供服务），后面的请求都直接落到了数据库上，造成数据库短时间内承受大量请求，导致服务崩溃

解决方法：

1. **集群**：采用Redis服务器集群，单个节点宕机依然保持服务的整体可用
2. **限流**：对大量请求限流，对服务降级
3. **随机失效**：随机设置缓存失效时间
4. **多级缓存**：构建多级缓存，在存储层前多加一层屏障，降低请求直达存储层的几率
...
在分布式应用中，可能多个进程同时修改数据，导致冲突



- **同步锁**：在多个**线程**都能访问到的地方，做一个标记以标识该数据的访问权限
- **分布式锁**：在多个**进程**都能访问到的地方，做一个标记以标识该数据的访问权限



分布式锁实现方式：基于数据库、Redis、Zookeeper等



Redis实现分布式锁的原则：

- 安全：独享，任意时刻只有一个客户端持有锁
- 活性A：无死锁，即使持有锁的客户端崩溃，锁仍然可以被获取
...
- 单Redis实例：
  1. 通过命令获取锁
  2. 通过Lua脚本释放锁
- 多Redis实例：Redlock算法（Java版本的库为Redisson）
