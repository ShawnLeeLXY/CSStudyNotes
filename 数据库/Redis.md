...
1. RDB：在一定的间隔时间中，检测key的变化情况，满足触发条件则创建快照，快照可以在本地持久化，也可以复制到其他服务器上
...
1. 先删除cache，成功则更新DB，否则不断做失败重试，一段时间还未成功则返回更新失败的异常
2. 旁路缓存模式：先更新DB，然后直接删除cache，增加cache更新重试机制，若不断更新失败则加入队列，直到cache服务可用后再将队列中的key删除
