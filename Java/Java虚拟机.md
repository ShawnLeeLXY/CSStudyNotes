...
非阻塞同步：如果没有其他线程争用共享数据，那操作就直接成功了；如果共享的数据的确被争用，产生了冲突，那再进行其他的补偿措施，最常用的补偿措施是不断地重试，直到出现没有竞争的共享数据为止

使用非阻塞同步的代码也叫**无锁同步编程**



CAS处理器指令：比较并交换（Compare-and-Swap），Java可以采用
...
JDK6中高效并发是一项重要改进，实现了各种锁优化技术



**自旋锁**

概念：多个线程并行执行，让后面请求锁的线程执行一个忙循环（自旋），看持有锁的线程能否很快释放，若多次自旋后还未获得锁，则阻塞
...
概念：通过CAS操作将对象的Mark Word（对象头的第一部分）更新为指向Lock Record的指针以此加锁，或通过CAS操作解锁

如果没有竞争，轻量级锁通过CAS操作成功比卖你了使用互斥量的开销；若存在锁竞争，则除了互斥量本身的开销外，还发生了CAS操作的开销



**偏向锁**

概念：在无竞争的情况下把整个同步都消除掉
